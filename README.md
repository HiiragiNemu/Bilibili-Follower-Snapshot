...
> **公开脚本与测试版都执行严格完整性校验。**
> 公开脚本要求扫描前总数、名单接口总数、扫描后总数一致，并且唯一 UID 数精确等于该总数；移动脚本与 probe v2 还记录逐页总数一致性。比较入口会再次核对双方完整性、目标 UID、计数和 UID 唯一性。
...
- 当前快照不完整时停用旧快照导入；即使通过代码触发文件事件，也会在计算差集前停止比较。
...
## 2026-08-08 自有账号 1000+ 回归证据

自有账号快照新增了超过 1000 的直接证据：`/x/relation/fans` 报告总数 1065，只取得 1000 个唯一 UID，第 21 页为空。旧版公开脚本随后错误生成了差集；脱敏记录见 [`research/evidence/self-over-1000-evidence-2026-08-09.md`](./research/evidence/self-over-1000-evidence-2026-08-09.md)。该 JSON 没有记录浏览器精确版本，亦不替代 Chrome/Edge 的完整 probe 复测。
...
这些项目描述测试分支中的 probe v2 行为。2026-08-03 浏览器证据对应的快照脚本 blob 为 `431d1b06996495670f553cc052433df626e7fcf9`。该轮 Chrome/Edge JSON 支持接口响应与集合结论，但原始 JSON 没有写入 commit 或脚本 hash；文件输入修复另由回归测试与当次页面内观察验证。新版手动启动器、脱敏下载和严格一致性规则由自动化测试覆盖，均与浏览器接口实测分开标注。
...
- 平台清理；
- 你移除了粉丝；
- 旧版本曾让服务端明细停线形成的不完整快照进入差集。

当前实现只在新旧快照均为 `complete: true`、目标 UID 相同、数组 UID 有效且唯一、报告计数与唯一 UID 数精确一致时执行差集。不完整快照不会生成比较结果，因此窗口外的早期粉丝不会再仅因 1000 人明细停线而被列为“关系已消失”。即使比较通过，主动取关、注销、封禁、拉黑、移除和平台清理仍具有相同的集合表现。
...
| `/x/relation/fans` 普通/官方游标 | 948 人自有样本完整；2026-08-08 的 1065 人自有样本普通分页取得 1000 后第 21 页为空；公开大样本取得 100 | 目标、模式和会话相关，需依赖本轮返回与完整性校验 |
| 官方 H5 客户端 | 源码设置本人 250、他人 100 的 UI 停线 | 客户端 `no_more` 条件，不是独立的服务端上限证据 |
| 创作中心统计 | 返回聚合计数、趋势和排行 | 没有发现完整粉丝身份或历史取关身份流水 |

脚本仍保留 1000 作为已知明细边界的保守诊断值。公开脚本仅在扫描前、名单接口、扫描后三个总数一致，且唯一实取数与该总数精确相等时写入 `complete: true`。若总数是 1050 而最终仅取得 1000，输出会记录 `exportedUniqueTotal: 1000`、`finalReportedTotal: 1050`、`complete: false` 并停用比较；这描述本轮结果，不把某条路径在某次采集中的停线外推为永久规则。

## 尚未覆盖

- 在粉丝总数超过 1000 的自有账号上完成同一套 Chrome 与 Edge 全路线 probe，并记录浏览器版本和集合哈希。
...
- [`research/evidence/self-over-1000-evidence-2026-08-09.md`](./research/evidence/self-over-1000-evidence-2026-08-09.md)：自有账号 1065/1000 停线和旧版误比较的脱敏证据摘要。
- [`tests/file-input-handler-regression.test.mjs`](./tests/file-input-handler-regression.test.mjs)：文件输入生命周期、完整性硬闸、UID 与计数校验回归测试。
- [`tests/snapshot-completeness-regression.test.mjs`](./tests/snapshot-completeness-regression.test.mjs)：公开脚本严格完整性与导入按钮状态回归测试。
